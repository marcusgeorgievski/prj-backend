...
module.exports = async (req, res, next) => {
  try {
    ...
    if (!name || !assessmentId) {
      ...
      return res.status(400).json({ error: "title and assessmentId are required" })
    }

    const updatedAssessment = await updateAssessment(assessmentId, name, description, dueDate, status, weight, classId)
